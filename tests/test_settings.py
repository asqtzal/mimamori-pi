...
class TestSettings:
    ...
    def test_default_values(self, tmp_path: Path) -> None:
        ...
        with patch.dict(os.environ, {}, clear=True):
            # .envファイルを読み込まないように存在しないファイルを指定
            settings = Settings(env_file=tmp_path / ".env.nonexistent")
    ...
    def test_debug_from_env(self, tmp_path: Path) -> None:
        """環境変数からDEBUGを読み込む."""
        test_cases = [
            ("true", True),
            ("True", True),
    ...
    def test_camera_resolution_from_env(self, tmp_path: Path) -> None:
        """環境変数からカメラ解像度を読み込む."""
        with patch.dict(os.environ, {"CAMERA_RESOLUTION": "1280x720"}, clear=False):
            settings = Settings(env_file=tmp_path / ".env.nonexistent")
            assert settings.CAMERA_RESOLUTION == (1280, 720)

    def test_camera_resolution_default(self, tmp_path: Path) -> None:
        """カメラ解像度のデフォルト値を確認."""
        with patch.dict(os.environ, {}, clear=True):
            settings = Settings(env_file=tmp_path / ".env.nonexistent")
            assert settings.CAMERA_RESOLUTION == (640, 480)

    def test_camera_resolution_invalid(self, tmp_path: Path) -> None:
        """無効な解像度値はデフォルト値を使用."""
